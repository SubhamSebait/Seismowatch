...
def predict_named(name):
    ...
    try:
        ...
        recent = [{
            ...
            'time':  datetime.utcfromtimestamp(f['properties']['time']/1000).strftime('%Y-%m-%d %H:%M'),
...
def active_cities():
    try:
        ...
        feats = resp.json().get('features', [])
        regions = defaultdict(lambda: {'count':0,'max_mag':0,'events':[],'lat':0,'lng':0})
        for f in feats:
            p      = f['properties']
        ...
        sorted_regions = sorted(
            ...
            key=lambda x: x['max_mag'], reverse=True
        )[:15]
        for r in sorted_regions:
            r['events'] = sorted(r['events'],key=lambda x:x['mag'],reverse=True)[:5]
        return jsonify({
...
# -- POST /api/predict/retrain -- retrain with feedback --------------------
@predict_bp.route('/retrain', methods=['POST'])
def retrain():
    try:
        from ml.retrain import retrain_with_feedback
        result = retrain_with_feedback()
        return jsonify({'status': 'retrained', **result})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


# -- POST /api/predict/update-cities -- update city risks with live data ----
@predict_bp.route('/update-cities', methods=['POST'])
def update_cities():
    try:
        from ml.forecaster import update_city_risks
        result = update_city_risks()
        return jsonify({'status': 'updated', 'cities': result})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


# -- GET /api/predict/city-risks -- get live city risks from MongoDB ---------
@predict_bp.route('/city-risks', methods=['GET'])
def get_city_risks():
    try:
